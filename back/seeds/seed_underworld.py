...
# List of Underworld movie IMDb IDs
underworld_movies = [
    "tt0320691",  # Underworld (2003)
    "tt0401855",  # Underworld: Evolution (2006)
    "tt0834001",  # Underworld: Rise of the Lycans (2009)
    "tt1496025",  # Underworld: Awakening (2012)
    "tt3717252"   # Underworld: Blood Wars (2016)
]


def seed_underworld_collection():
    """Seed the database with Underworld movies and create a playlist"""
...
Hello, welcome to this Underworld movie.
...
"""
    ...
    db = SessionLocal()
    try:
        print("Starting to seed Underworld collection...")
        
        # Dictionary to store created videos
        created_videos = {}
        
        # 1. Create the videos
        for imdb_id in underworld_movies:
        ...
        # 2. Create the Underworld playlist
        print("\nCreating Underworld playlist...")
        
        # Check if playlist already exists
        existing_playlist = db.query(Playlist).filter(Playlist.name == "Underworld Collection").first()
        if existing_playlist:
            print("Underworld playlist already exists. Clearing videos...")
            ...
            # Create new playlist
            playlist = Playlist(
                name="Underworld Collection",
                description="Complete collection of Underworld movies, featuring the war between vampires and lycans."
            )
            db.add(playlist)
            db.commit()
            db.refresh(playlist)
            print("Created new Underworld playlist")
        
        # Add Underworld movies to playlist
        videos_added = 0
        for imdb_id in underworld_movies:
            if imdb_id in created_videos:
                ...
                videos_added += 1
                print(f"Added {video.title} ({video.year}) to Underworld playlist")
        
        db.commit()
        print(f"Successfully added {videos_added} videos to Underworld playlist")
        ...
        print("\nFinished seeding Underworld collection!")
    except Exception as e:
        print(f"Error seeding Underworld collection: {str(e)}")
        import traceback
        traceback.print_exc()
    finally:
        db.close()
